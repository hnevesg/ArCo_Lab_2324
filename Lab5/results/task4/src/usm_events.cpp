...
int main() {
  ...
  auto event1 = q.parallel_for(range<1>(N), [=](id<1> i) { data[i] += 2; });

  auto event2 = q.submit([&](handler &h){
	  h.depends_on(event1);
	  q.parallel_for(range<1>(N), [=](id<1> i) { data[i] += 3; });
  });
  
  q.submit([&](handler &h){
	  h.depends_on(event2); // if event 2 depends on 1, event 3 implicitly waits for 1 too
