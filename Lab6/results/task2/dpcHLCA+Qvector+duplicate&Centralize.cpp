...
void duplicateAndCentralizeImg(unsigned short *ImgRef, short *Img, int *centroid)
{
  ...
  cl::sycl::buffer <int, 1> buff_centrd{centroid, cl::sycl::range<1>(BANDS)};
	...
	q.submit([&] (cl::sycl::handler& h){
	  ...
	  //Accessors	
    cl::sycl::accessor accessorImgRef{buff_imgRf, h, cl::sycl::read_only};
		cl::sycl::accessor accessorImg{buff_img, h, cl::sycl::read_write};
		cl::sycl::accessor accessorCentroid{buff_centrd, h, cl::sycl::read_write};

	  	h.parallel_for(cl::sycl::range<1>(BLOCK_SIZE), [=](auto& idx){
            for(int band=0; band<BANDS; band++){
              accessorImg[idx*BANDS + band] = ((short)(accessorImgRef[idx*BANDS + band]) - (short)(accessorCentroid[band]))<<2;
            }
 	  	});
  	});
