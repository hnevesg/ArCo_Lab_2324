...
int main(int argc, char* argv[])
{
  ...
  mkdir("output", 0777);
	...
	cl::sycl::queue q(selector);	
      ...
      std::cout << "Device maximum nº of parallel compute units: " << q.get_device().get_info<cl::sycl::info::device::max_compute_units>() << std::endl;
...
void runCompressor(unsigned short *input_img, char* OutputFileName)		
{
  ...
  for(int blockIndex = 0; blockIndex < nBlocks; blockIndex++){ //<nblocks
