...
{
 ANNOTATE_SITE_BEGIN(outer1);
 #pragma omp parallel for
  for(int band=0; band<BANDS; band++){
    centroid[band] = 0;
    ANNOTATE_ITERATION_TASK(transform1)
    #pragma omp parallel for reduction (+: centroid[band])
...
#pragma omp parallel for
  for(int pixel=0; pixel<BLOCK_SIZE; pixel++){
    ...
    actualBrightness = 0;
#pragma omp parallel for reduction(+: actualBrightness)
    for(int band=0; band<BANDS; band++){	
...
void projectingImg(short *Img, int *projection, int *uVector)
{
 ...
 ANNOTATE_SITE_BEGIN(outer2);
 #pragma omp parallel for
  for(int pixel=0; pixel<BLOCK_SIZE; pixel++){
    projectionValueLong = 0;
     ANNOTATE_ITERATION_TASK(transform2);
     #pragma omp parallel for reduction(+: projectionValueLong)
    for(int band=0; band<BANDS; band++){
